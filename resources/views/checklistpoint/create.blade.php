...
<form class="max-w-sm mx-auto" action="{{ route('checklistpoints.store') }}" method="POST">
  <div class="mb-5">
    <label for="checklist_section_id" class="block mb-2.5 text-sm font-medium text-heading">Категория требований</label>
    <select name="checklist_section_id" id="checklist_section_id" class="block w-full px-3 py-2.5 ...">
        <option value="" disabled selected>Выберите категорию</option>
        @foreach($checklistsections as $checklistsection)
            <option value="{{ $checklistsection->id }}">{{ $checklistsection->name }}</option>
        @endforeach
    </select>
  </div>
  <div class="mb-5">
